...
`useOnScreen` is the boolean-only visibility hook in `use-kit`. It is built on `useIntersectionObserver` but keeps the API focused when you already have a ref and only need a yes-or-no answer.
