...
const EXPORTS: { path: string; note: string }[] = [
  { path: "react-rsc-kit", note: "Default entry — shared utilities and components." },
  { path: "react-rsc-kit/server", note: "Server-only / RSC-friendly imports." },
  { path: "react-rsc-kit/client", note: "Client components, hooks, and browser helpers." },
  { path: "react-rsc-kit/hooks", note: "Hooks-only barrel for smaller client bundles." },
];
        ...
        <div className="grid gap-10 sm:grid-cols-2">
          <div>
            ...
            <ul className="mt-2 divide-y divide-[var(--uk-border-subtle)] border-y border-[var(--uk-border-subtle)]">
              {HOOK_PAGES.map((name) => (
                <li key={name}>
                  <Link href={`/hooks/${name}/`} className="react-rsc-kit-doc-nav-link">
                    <code>{name}</code>
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="mt-2 divide-y divide-[var(--uk-border-subtle)] border-y border-[var(--uk-border-subtle)]">
              {COMPONENT_PAGES.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="react-rsc-kit-doc-nav-link">
                    {item.label}
                  </Link>
                </li>
              ))}
            ...
            <ul className="mt-2 space-y-1 text-sm text-[var(--uk-text-muted)]">
              <li>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="react-rsc-kit-doc-inline-link"
                >
                  npm package
                </a>
              </li>
